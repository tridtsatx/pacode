//! `codeapp daemon`: status and stop.
...
pub fn run(action: DaemonAction, socket: PathBuf) -> anyhow::Result<()> {
    ...
    rt.block_on(async move {
        match action {
            DaemonAction::Status => {
                match codeapp_client::daemon_status(&socket, codeapp_config::APP_VERSION).await {
                    Some(status) => {
            ...
            DaemonAction::Stop { force } => {
                codeapp_client::stop_daemon(&socket, codeapp_config::APP_VERSION, force)
                    .await
