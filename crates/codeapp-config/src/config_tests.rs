...
fn env_overrides_invalid_effort() {
    let mut cfg = Config::default();
    let err = apply_env_overrides(&mut cfg, [("CODEAPP_EFFORT", "ultra")])
        .expect_err("invalid effort must fail");

    match err {
        ConfigError::Env { var, value } => {
            assert_eq!(var, "CODEAPP_EFFORT");
...
fn env_overrides_invalid_mode() {
    let mut cfg = Config::default();
    let err = apply_env_overrides(&mut cfg, [("CODEAPP_MODE", "godmode")])
        .expect_err("invalid mode must fail");

    match err {
        ConfigError::Env { var, value } => {
            assert_eq!(var, "CODEAPP_MODE");
...
fn paths_under_layout() {
    ...
    assert_eq!(paths.db_file(), root.join("data/codeapp.db"));
...
fn logger_writes_line_containing_message() {
    ...
    let record = log::Record::builder()
        ...
        .target(target_name)
        .module_path(Some("codeapp_config::tests"))
