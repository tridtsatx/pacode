...
use codeapp_tools::{Tool, ToolRegistry};
...
impl Core {
    ...
    pub(crate) async fn get_mcp_tools(&self) -> Vec<Arc<dyn Tool>> {
        ...
        let tools = codeapp_tools::builtin::mcp::mcp_tools(self.deps.mcp.clone()).await;
            ...
            Request::GetHistory {
                ...
                if tail_items.len() >= limit as usize || !has_more {
                    ...
                    let older_messages = session
                        ...
                        .await
                        .unwrap_or_default();
                    let older_tuples: Vec<(u64, Arc<codeapp_types::Message>)> = older_messages
                        .into_iter()
