...
pub fn split_for_compaction(
    ...
    while cut > 0 {
        if messages[cut].role == codeapp_types::Role::Tool
            || (messages[cut - 1].has_tool_uses()
                && cut < messages.len()
                && messages[cut].role == codeapp_types::Role::Tool)
        {
            cut -= 1;
        } else {
            break;
...
pub fn build_summary_request(
    ...
    use codeapp_types::{ContentBlock, Role};
    let mut out = Vec::new();
...
pub async fn compact(session: &Arc<Session>, agent: &Arc<Agent>) -> Result<bool, CoreError> {
    ...
    while let Some(ev_res) = stream.next().await {
        match ev_res {
            Ok(codeapp_types::StreamEvent::TextDelta { text }) => {
...

    {
        ...
        let summary_tokens = codeapp_types::estimate_tokens(&summary);
    ...
    let notice_item = codeapp_types::TranscriptItem {
        ...
        kind: codeapp_types::TranscriptKind::Notice {
            level: codeapp_types::ToastLevel::Info,
    ...
    session
        .events
        .emit(codeapp_types::Event::ItemAdded(notice_item));
...
mod tests {
    use super::*;
    use codeapp_types::{CallId, ContentBlock, Message, Role};
