...
use codeapp_types::{Attach, SessionId};
...
pub(crate) async fn open_session(core: &Core, attach: Attach) -> Result<SessionId, CoreError> {
    match attach {
        Attach::Latest { cwd } => {
            let filter = codeapp_store::SessionFilter {
                cwd: Some(cwd.clone()),
                limit: 1,
            };
        ...
        Attach::New {
            ...
            let meta = codeapp_types::SessionMeta {
                id: id.clone(),
                name: None,
                cwd: cwd.clone(),
                git_branch,
            ...
            let main_info = codeapp_types::AgentInfo {
                id: codeapp_types::AgentId::main(),
                name: "main".to_string(),
                kind: codeapp_types::AgentKind::Main,
            ...
            agents.insert(codeapp_types::AgentId::main(), main_agent);

            let session = Arc::new(Session {
                id: id.clone(),
                meta: RwLock::new(meta),
                agents: RwLock::new(agents),
                plan: RwLock::new(codeapp_types::Plan::default()),
                usage: RwLock::new(codeapp_types::UsageTotals::default()),
        ...
        Attach::Resume {
            ...
            for mut task in stored_tasks {
                if task.status == codeapp_types::TaskStatus::Running {
                    task.status = codeapp_types::TaskStatus::Killed;
            ...
            let compaction = core
                .deps
                .store
                .load_compaction(&session_id, &codeapp_types::AgentId::main())
                .await?;
            ...
            let all_messages = core
                .deps
                .store
                .load_messages(&session_id, &codeapp_types::AgentId::main())
                .await?;
            ...
            let est_tokens = messages_after
                ...
                + summary
                    .as_deref()
                    .map(codeapp_types::estimate_tokens)
                    .unwrap_or(0);
            ...
            for mut sub_info in stored_agents {
                ...
                if sub_info.status.is_live() {
                    sub_info.status = codeapp_types::AgentStatus::Finished;
                    let _ = core
                        .deps
                        .store
