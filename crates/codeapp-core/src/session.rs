...
impl Session {
    ...
    pub fn is_busy(&self) -> bool {
        ...
        self.tasks_of_session()
            .iter()
            .any(|t| t.status == codeapp_types::TaskStatus::Running)
    }
    ...
    pub async fn submit_user_message(self: &Arc<Self>, text: String) -> Result<(), CoreError> {
        ...
        let (seq, arc_msg) = main
            ...
            .push(codeapp_types::Message::user(&text));
        self.store
        ...
        let item = codeapp_types::TranscriptItem {
            ...
            kind: codeapp_types::TranscriptKind::User { text: text.clone() },
        };
        main.transcript
            .lock()
            .unwrap_or_else(|p| p.into_inner())
            .upsert(item.clone());
        self.events.emit(codeapp_types::Event::ItemAdded(item));
    ...
    pub async fn spawn_agent(self: &Arc<Self>, spec: AgentSpec) -> Result<AgentId, CoreError> {
        ...
        let tools = if let Some(tool_names) = spec.tools {
            ...
            let default_names = codeapp_tools::subagent_tool_names(&self.tools);
        ...
        history.push(codeapp_types::Message::user(&spec.prompt));
        ...
        let info = AgentInfo {
            ...
            status: codeapp_types::AgentStatus::Idle,
            activity: None,
            started_at_ms: now,
            finished_at_ms: None,
            tokens_in: 0,
        ...
        self.events.emit(codeapp_types::Event::AgentAdded(info));
    ...
    pub async fn stop_agent(&self, id: &AgentId) -> Result<(), CoreError> {
        ...
        agent.set_status(codeapp_types::AgentStatus::Stopped, None);
        let info = agent.info();
        let _ = self
            .store
            .upsert_agent(&self.id, &info, agent.prompt.as_deref())
            .await;
        self.events.emit(codeapp_types::Event::AgentUpdated(info));
        Ok(())
    }
    ...
    pub fn set_plan(&self, plan: Plan) {
        ...
        self.events.emit(codeapp_types::Event::PlanUpdated(plan));
    }
    ...
    pub fn record_usage(&self, agent: &AgentId, usage: codeapp_types::Usage, context_tokens: u32) {
        ...
        let totals = {
            ...
            u.last_activity_ms = codeapp_types::now_ms();
            if let Some(p) = pricing {
                let usage_struct = codeapp_types::Usage {
        ...
        self.events.emit(codeapp_types::Event::UsageUpdated(totals));
    }
    ...
    pub fn touch(&self) {
        let meta = {
            ...
            m.updated_at_ms = codeapp_types::now_ms();
            m.clone()
        };
        let store = self.store.clone();
        ...
        self.events.emit(codeapp_types::Event::SessionUpdated(meta));
    }
}
