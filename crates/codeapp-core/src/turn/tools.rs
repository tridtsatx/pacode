        ...
        match agent.tools.get(&call.name) {
            Some(tool) => {
                ...
                let tool_res = tokio::select! {
                    _ = cancel.cancelled() => Err(codeapp_tools::ToolError::Cancelled),
                    res = call_fut => res,
                };
                match tool_res {
                    Ok(output) => {
                    ...
                    Err(codeapp_tools::ToolError::Cancelled) => {
                        let stub = Message::tool_result(call.call_id.clone(), "cancelled", true);
                        let (seq, arc_stub) = agent
                            .history
                            .lock()
                    ...
                    Err(codeapp_tools::ToolError::Denied(reason)) => (
