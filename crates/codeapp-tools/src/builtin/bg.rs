    ...
    async fn call(&self, input: Value, ctx: &ToolCtx) -> Result<ToolOutput, ToolError> {
        ...
        match args.action.as_str() {
            "list" => {
                ...
                for task in &tasks {
                    let status_str = match task.status {
                        codeapp_types::TaskStatus::Running => "running",
                        codeapp_types::TaskStatus::Completed => "completed",
                        codeapp_types::TaskStatus::Failed => "failed",
                        codeapp_types::TaskStatus::Killed => "killed",
                    };
            ...
            "status" => {
                ...
                let dur_s = info.duration_ms(codeapp_types::now_ms()) / 1000;
                let mut out = format!(
                    "Task: {}\nLabel: {}\nCommand: {}\nStatus: {:?}\nDuration: {}s\nExit code: {}\nCwd: {}\nSpool: {}\nBytes: {}",
                    info.id,
                    info.label,
            ...
            "progress" => {
                ...
                let id = TaskId::new(task_id_str);
                let progress = codeapp_types::TaskProgress {
                    current: args.current,
                    total: args.total,
                    percent: args.percent.map(|p| p as f32),
                    message: args.message,
                    source: codeapp_types::ProgressSource::Reported,
