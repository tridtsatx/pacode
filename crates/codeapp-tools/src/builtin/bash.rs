...
impl Tool for BashTool {
    ...
    async fn call(&self, input: Value, ctx: &ToolCtx) -> Result<ToolOutput, ToolError> {
        ...
        let assessment = codeapp_risk::classify(&args.command, &ctx.cwd);
        if ctx.mode == codeapp_types::Mode::Plan && !codeapp_risk::is_read_only(&args.command) {
        ...
        let label = args
            .label
            .clone()
            .unwrap_or_else(|| codeapp_types::task_label_from_command(&args.command));
        ...
        let mut spec = codeapp_exec::TaskSpec::new(
            ctx.session.clone(),
            ctx.agent.clone(),
            args.command.clone(),
            ctx.cwd.clone(),
