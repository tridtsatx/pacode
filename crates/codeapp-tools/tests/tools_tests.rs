...
use codeapp_exec::{TaskManager, TaskSpec};
use codeapp_tools::Tool;
use codeapp_tools::builtin::agent::AgentTool;
use codeapp_tools::builtin::bash::BashTool;
use codeapp_tools::builtin::bg::BgTool;
use codeapp_tools::builtin::edit::EditTool;
use codeapp_tools::builtin::glob::GlobTool;
use codeapp_tools::builtin::grep::GrepTool;
use codeapp_tools::builtin::ls::LsTool;
use codeapp_tools::builtin::multi_edit::MultiEditTool;
use codeapp_tools::builtin::plan::PlanTool;
use codeapp_tools::builtin::read::ReadTool;
use codeapp_tools::builtin::write::WriteTool;
use codeapp_tools::host::{AgentSpec, PermissionDraft, ToolCtx, ToolHost, WaitOutcome};
use codeapp_tools::output::ToolError;
use codeapp_types::{
...
impl ToolHost for StubHost {
    ...
    async fn spawn_agent(&self, spec: AgentSpec) -> Result<AgentId, ToolError> {
        ...
        let info = AgentInfo {
            id: id.clone(),
            name,
            kind: codeapp_types::AgentKind::Sub,
            status: AgentStatus::Thinking,
            activity: None,
            started_at_ms: codeapp_types::now_ms(),
            finished_at_ms: None,
            tokens_in: 0,
            tokens_out: 42,
            model: spec
...
async fn test_edit_unique_ambiguous_replace_all_atomic() {
    ...
    assert!(!tmp.path().join("file.txt.codeapp-tmp").exists());
...
async fn test_webfetch_url_validation() {
    ...
    let tool = codeapp_tools::builtin::webfetch::WebFetchTool;
...
async fn test_mcp_tools_proxy() {
    ...
    let fake_mcp_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .unwrap()
        .join("codeapp-mcp/tests/fake_mcp.py");
    ...
    let tools = codeapp_tools::builtin::mcp::mcp_tools(pool.clone()).await;
