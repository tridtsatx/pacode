...
async fn build_test_core(tmp: &tempfile::TempDir, config: Arc<Config>) -> Arc<Core> {
    let paths = codeapp_config::Paths::under(tmp.path());
...
async fn bind_socket_removes_stale_socket_file() {
    ...
    match err {
        codeapp_daemon::DaemonError::SocketBusy { path } => {
...
async fn hello_with_protocol_999_errors() {
    ...
    config
        .providers
        .insert("mock".to_string(), codeapp_types::ProviderConfig::default());
...
async fn full_flow_hello_attach_message_events_snapshot_shutdown() {
    ...
    while !saw_turn_ended {
        ...
        if let ServerMessage::Event { seq: _, event } = msg {
            match event {
                codeapp_types::protocol::Event::TextDelta { .. } => {
                    saw_text_delta = true;
                }
                codeapp_types::protocol::Event::TurnEnded { .. } => {
                    saw_turn_ended = true;
                }
                _ => {}
            }
...
async fn build_core_with_mock_provider() {
    ...
    // Set CODEAPP_MOCK_PROVIDER
    unsafe {
        std::env::set_var("CODEAPP_MOCK_PROVIDER", "1");
    }
    ...
    unsafe {
        std::env::remove_var("CODEAPP_MOCK_PROVIDER");
    }
